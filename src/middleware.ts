...
export default withAuth(async (req: NextRequest) => {
  ...
  if (ADMIN_ROUTES.includes(req.nextUrl.pathname) && token?.role !== "ADMIN")
    // The only way I found to send errors to the page using the middleware is to send a parameter in the URL, and it is the default behavior of next-auth
...
export const config = { matcher: ["/home", "/dashboard"] }; // Was gonna add ROUTES here
